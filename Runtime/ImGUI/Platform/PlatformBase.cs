...
namespace Espionage.Engine.ImGUI.Platform
{
	/// <summary>
	...
	internal class PlatformBase : IPlatform
	{
		...
		protected readonly PlatformCallbacks _callbacks = new();
		...
		protected void UpdateCursor( ImGuiIOPtr io, ImGuiMouseCursor cursor )
		{
			...
			_lastCursor = cursor;
			UnityEngine.Cursor.visible = cursor != ImGuiMouseCursor.None; // Hide cursor if ImGui is drawing it or if it wants no cursor.
			if ( _cursorShapes != null )
			{
				UnityEngine.Cursor.SetCursor( _cursorShapes[cursor].Texture, _cursorShapes[cursor].Hotspot, CursorMode.Auto );
			}
		}
	}
}
